...
{
	size_t currentPos = file.tellg(); // Запазва текущата позиция на указателя във файла
	file.seekg(0, std::ios::end); // Прехвърля позицията на указателя в края на файла
	size_t fileSize = file.tellg(); // Взема размера на файла от текущата позиция на указателя
	file.seekg(currentPos); // Възстановява позицията на указателя до предишната запазена позиция
	return fileSize;
}
